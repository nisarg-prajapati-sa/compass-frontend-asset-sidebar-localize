...
import {
  getArticle,
  getArticleListingPageByTaxonomy,
  getArticles,
} from "@/loaders";
...
export default function Article() {
  ...
  const [articles, setArticles] = useState<Page.ArticlePage["articles"] | null>(
    null
  );
  const [relatedLinks, setRelatedLinks] = useState<
    Page.ArticleListingPage["entry"][] | []
  >([]);
  const { path, locale } = useRouterHook();

  const fetchData = async () => {
    try {
      const entryData: Page.ArticlePage["entry"] = await getArticle(
        path,
        locale
      );
      setData(entryData);
    ...
    try {
      if (data && data?.taxonomies?.length > 0) {
        if (show_related_links) {
          const listingData: Page.ArticleListingPage["entry"][] =
            await getArticleListingPageByTaxonomy(locale, data?.taxonomies);
          listingData && setRelatedLinks(listingData);
        }
        if (show_related_articles) {
          let articlesData: Page.ArticlePage["articles"] = await getArticles(
            locale,
            data?.taxonomies,
            7
          );
          articlesData = articlesData?.filter(
            (article) => article.uid !== data?.uid
          );
 ...
 console.log(cover_image?._metadata?.extensions)
  ...
  const relatedArticles =
    ...
    cards.splice(
      0,
      data?.related_articles?.number_of_articles &&
        data?.related_articles?.number_of_articles <= 6
