        ...
        const queries = query?.filterQuery?.map((q) => {
          if (typeof Object.values(q)?.[0] === "string") {
            return Stack.ContentType(contentTypeUid)
              .Query()
              .where(Object.keys(q)?.[0], Object.values(q)?.[0]);
          }
          return Stack.ContentType(contentTypeUid)
            .Query()
            .containedIn(Object.keys(q)?.[0], Object.values(q)?.[0]);
        });
...
export const getEntryByUID = async (
  ...
  try {
    ...
    if (entryQuery) {
      ...
      result = await entryQuery.includeFallback().toJSON().fetch();
      ...
      isEditButtonsEnabled &&
        addEditableTags(result, contentTypeUid, true, locale);
      return result;
    }
  } catch (error) {
    if (error?.error_message) throw new Error(JSON.stringify(error));
